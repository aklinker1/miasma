...
func (o *UpdateAppEnvURL) Build() (*url.URL, error) {
	...
	var _path = "/api/apps/{appName}/env"

	appName := o.AppName
	if appName != "" {
		_path = strings.Replace(_path, "{appName}", appName, -1)
	} else {
		return nil, errors.New("appName is required on UpdateAppEnvURL")
	}
...
func (o *UpdateAppEnvURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on UpdateAppEnvURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on UpdateAppEnvURL")
	}
