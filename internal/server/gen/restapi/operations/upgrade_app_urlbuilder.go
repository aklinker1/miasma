...
func (o *UpgradeAppURL) Build() (*url.URL, error) {
	...
	var _path = "/api/apps/{appName}/upgrade"

	appName := o.AppName
	if appName != "" {
		_path = strings.Replace(_path, "{appName}", appName, -1)
	} else {
		return nil, errors.New("appName is required on UpgradeAppURL")
	}
	...
	var newImageQ string
	if o.NewImage != nil {
		newImageQ = *o.NewImage
	}
	if newImageQ != "" {
		qs.Set("newImage", newImageQ)
	}
...
func (o *UpgradeAppURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on UpgradeAppURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on UpgradeAppURL")
	}
