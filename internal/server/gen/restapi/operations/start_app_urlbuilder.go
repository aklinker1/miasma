...
func (o *StartAppURL) Build() (*url.URL, error) {
	...
	var _path = "/api/apps/{appName}/start"

	appName := o.AppName
	if appName != "" {
		_path = strings.Replace(_path, "{appName}", appName, -1)
	} else {
		return nil, errors.New("appName is required on StartAppURL")
	}
...
func (o *StartAppURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on StartAppURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on StartAppURL")
	}
