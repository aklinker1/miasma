...
func (o *StopAppURL) Build() (*url.URL, error) {
	...
	var _path = "/api/apps/{appName}/stop"

	appName := o.AppName
	if appName != "" {
		_path = strings.Replace(_path, "{appName}", appName, -1)
	} else {
		return nil, errors.New("appName is required on StopAppURL")
	}
...
func (o *StopAppURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on StopAppURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on StopAppURL")
	}
